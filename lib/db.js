...
export async function getQuestionsForRetrieval(company, limit = 200) {
  ...
  async function fetchStratified(withCompanyFilter) {
  ...
  let questions = await fetchStratified(true)
  if (questions.length >= 50) {
    return { questions, companyMatch: true }
  }

  // Fallback: no company filter, up to 200 rows
  questions = await fetchStratified(false)
  return { questions: questions.slice(0, limit), companyMatch: false }
}
