...
export async function savePrediction(data) {
  ...
  const id = crypto.randomUUID()
  await db.execute({
    sql: `INSERT INTO predictions (id, company, role_level, round_type, jd_text, cv_text, result, outcome)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [
      id,
      data.company || '',
...
export async function getPredictions() {
  ...
  const db = getDb()
  const res = await db.execute({
    sql: 'SELECT id, company, role_level, round_type, outcome, created_at, result FROM predictions ORDER BY created_at DESC',
    args: []
  })
  return res.rows
}
