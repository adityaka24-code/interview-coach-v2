...
export async function initSchema() {
  ...
  // One-time migration: drop open_jobs if it was created with UNIQUE on link
  try {
    const res = await db.execute({ sql: "SELECT sql FROM sqlite_master WHERE type='table' AND name='open_jobs'", args: [] })
    if (res.rows[0]?.sql?.includes('UNIQUE')) {
      await db.execute({ sql: 'DROP TABLE open_jobs', args: [] })
      await db.execute({ sql: `CREATE TABLE open_jobs (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        company TEXT NOT NULL,
        company_rank INTEGER,
        title TEXT NOT NULL,
  ...
  } catch (e) { console.error('[db] open_jobs schema migration failed:', e.message) }
  ...
  const addCols = [
    ...
    // pm_questions embedding migrations
    "ALTER TABLE pm_questions ADD COLUMN embedding TEXT DEFAULT NULL",
    "ALTER TABLE pm_questions ADD COLUMN embedding_model TEXT DEFAULT NULL",
    "ALTER TABLE pm_questions ADD COLUMN experience_years TEXT DEFAULT ''",
    ...
    "ALTER TABLE predictions ADD COLUMN low_confidence INTEGER DEFAULT 0",
    "ALTER TABLE pm_questions ADD COLUMN confirmation_count INTEGER DEFAULT 0",
    "ALTER TABLE pm_questions ADD COLUMN source TEXT DEFAULT 'lewis_lin'",
    "ALTER TABLE pm_questions ADD COLUMN source_url TEXT DEFAULT NULL",
    "ALTER TABLE pm_questions ADD COLUMN source_label TEXT DEFAULT NULL",
    "ALTER TABLE prediction_feedback ADD COLUMN source TEXT DEFAULT NULL",
  ]
...
export async function getPmQuestions({
  company, role, search, questionType, source, sortBy = 'recency'
} = {}) {
  await initSchema()
  const db = getDb()
  ...
  if (company) {
    sql += ' AND LOWER(TRIM(company)) LIKE LOWER(TRIM(?))'
    args.push(`%${company}%`)
  }
  if (role) {
    sql += ' AND LOWER(TRIM(job_title)) LIKE LOWER(TRIM(?))'
    args.push(`%${role}%`)
  }
  if (search) {
    sql += ' AND LOWER(question) LIKE LOWER(?)'
    args.push(`%${search}%`)
  }
  if (questionType) {
  ...
  sql += sortBy === 'frequency'
    ? ' ORDER BY confirmation_count DESC, timestamp DESC'
...
export async function getFilterOptions() {
  ...
  const [companies, roles, locations, qCompanies, pmCompanies, sources, salarySources] = await Promise.allSettled([
    ...
    db.execute({ sql: "SELECT DISTINCT company FROM pm_questions WHERE company!='' ORDER BY company", args: [] }),
    db.execute({ sql: `SELECT DISTINCT source FROM questions UNION SELECT DISTINCT source FROM pm_questions WHERE source IS NOT NULL ORDER BY source`, args: [] }),
  ...
  const allQCompanies = [
    ...rows(qCompanies).map(r=>r.company),
    ...rows(pmCompanies).map(r=>r.company),
  ].filter(Boolean)
  const uniqueQCompanies = [...new Set(allQCompanies)].sort()
  return {
    companies: rows(companies).map(r=>r.company),
    roles:     rows(roles).map(r=>r.role),
    locations: rows(locations).map(r=>r.location),
    qCompanies: uniqueQCompanies,
...
export async function savePrediction(data) {
  try {
    ...
    console.error('[savePrediction] initSchema failed:', err.message)
  ...
  try {
    ...
    console.error('[savePrediction] getDb failed:', err.message)
  ...
  if (!data.jdText && !data.cvText && !data.result) {
    console.warn('[savePrediction] Warning: saving prediction with minimal data', { company, roleLevel, roundType })
  }
  ...
  try {
    ...
    console.error('[savePrediction] SELECT existing failed:', err.message)
    throw new Error(`Failed to check existing predictions: ${err.message}`)
  }

  const lowConfidence = data.lowConfidence ? 1 : 0

  const fullResult = {
    ...(data.result || {}),
    retrievedQuestions: data.retrievedQuestions || [],
    ...
    try {
      const updateResult = await db.execute({
        sql: `UPDATE predictions
              SET jd_text=?, cv_text=?, result=?, outcome='pending', low_confidence=?, created_at=datetime('now')
              WHERE id=?`,
        args: [data.jdText || '', data.cvText || '', JSON.stringify(fullResult), lowConfidence, id],
      })
      console.log('[savePrediction] Updated existing prediction:', { id, rowsAffected: updateResult.rowsAffected })
    } catch (err) {
      console.error('[savePrediction] UPDATE existing failed:', err.message)
      throw new Error(`Failed to update prediction: ${err.message}`)
    }
    return id
  }
  ...
  try {
    const insertResult = await db.execute({
      sql: `INSERT INTO predictions (id, company, role_level, round_type, jd_text, cv_text, result, outcome, user_id, low_confidence)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [id, company, roleLevel, roundType, data.jdText || '', data.cvText || '',
             JSON.stringify(fullResult), 'pending', userId, lowConfidence],
    })
    console.log('[savePrediction] Created new prediction:', {
      id,
      company,
      roleLevel,
      roundType,
      userId,
      jdTextLength: (data.jdText || '').length,
      cvTextLength: (data.cvText || '').length,
      resultLength: JSON.stringify(fullResult).length,
      rowsAffected: insertResult.rowsAffected
    })
  } catch (err) {
    console.error('[savePrediction] INSERT new failed:', err.message, {
      id,
      company,
      roleLevel,
      roundType
    })
    throw new Error(`Failed to save prediction: ${err.message}`)
  }
...
/**
 * Debug function to verify predictions table and recent data.
 * Call this if you suspect predictions aren't being saved.
 */
export async function debugPredictions() {
  try {
    await initSchema()
    const db = getDb()
    ...
    const tableInfo = await db.execute({
      sql: "PRAGMA table_info(predictions)",
      args: []
    })
    console.log('[debug] Predictions table columns:', tableInfo.rows.map(r => r.name))

    // Check total count
    const countRes = await db.execute({
      sql: 'SELECT COUNT(*) as cnt FROM predictions',
      args: []
    })
    const count = countRes.rows[0]?.cnt || 0
    console.log('[debug] Total predictions in DB:', count)

    // Check recent ones
    const recentRes = await db.execute({
      sql: 'SELECT id, company, role_level, round_type, created_at FROM predictions ORDER BY created_at DESC LIMIT 5',
      args: []
    })
    console.log('[debug] 5 most recent predictions:', recentRes.rows)

    return {
      tableExists: true,
      columns: tableInfo.rows.map(r => r.name),
      totalCount: count,
      recent: recentRes.rows
    }
  } catch (err) {
    console.error('[debug] debugPredictions error:', err.message)
...
/**
 * Subsidiaries / product brands that should inherit the parent company's
 * question bank when they don't have enough data of their own.
 * Keys and values are already normalised (lowercase, no suffix).
 */
...
export async function getQuestionsForRetrieval(company, limit = 200) {
  await initSchema()
  if (_dbUnavailable) return { questions: [], companyMatch: false }
  const db = getDb()
  const normCompany = normaliseCompany(company)

  async function fetchStratifiedForCompany(companyName) {
    const results = await Promise.allSettled(
      PM_QUESTION_TYPES.map(type =>
        db.execute({
          sql: `SELECT question, question_type, company, job_title, experience_years, embedding, embedding_model, timestamp, confirmation_count, source, source_url, source_label
                FROM pm_questions
  ...
  async function fetchStratifiedGlobal() {
    const results = await Promise.allSettled(
      PM_QUESTION_TYPES.map(type =>
        db.execute({
          sql: `SELECT question, question_type, company, job_title, experience_years, embedding, embedding_model, timestamp, confirmation_count, source, source_url, source_label
  ...
  // 1. Try the exact company name first
  let rows = await fetchStratifiedForCompany(normCompany)

  // 2. If not enough, merge in parent/alias companies before giving up on company match
  if (rows.length < 8) {
    const aliases = COMPANY_ALIASES[normCompany] || []
    for (const alias of aliases) {
      const aliasRows = await fetchStratifiedForCompany(alias)
      rows = rows.concat(aliasRows)
    }
  }

  const questions = dedupe(rows)
  if (questions.length >= 8) {
    return { questions, companyMatch: true }
  }

  // 3. Final fallback: no company filter, global pool
  const globalRows = await fetchStratifiedGlobal()
  return { questions: dedupe(globalRows).slice(0, limit), companyMatch: false }
}

export async function savePredictionFeedback({ predictionId, questionText, wasAsked, interviewId }) {
  await initSchema()
  if (_dbUnavailable) return null
  const db = getDb()
  const id = crypto.randomUUID()
  await db.execute({
    sql: `INSERT INTO prediction_feedback (id, prediction_id, question_text, was_asked, interview_id, created_at)
          VALUES (?, ?, ?, ?, ?, datetime('now'))`,
    args: [id, predictionId || null, questionText || '', wasAsked ? 1 : 0, interviewId || null],
  })
  return id
}
